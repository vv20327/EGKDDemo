...
def main(args):
    ...
    feature, weights_lp, label, idx_train,  idx_test, n_feat, n_class = load_data()
    #feature表示特征
    print(idx_train)
    ...
    features_1, adj_1 = perturb_graph(feature, adj_lp, 0.2, 0.07, args)
    features_2, adj_2 = perturb_graph(feature, adj_lp, 0.4, 0.1, args)
    ...
    # make_graph_with_tSNE(feature, label, cl_model, adj_1, adj_2, model_path, idx)
    # # make_test_tSNE(feature, label, cl_model, adj_lp, adj_hp, model_path, idx_test)
    print(111111111111111111111111111111111111111111111111111111111111111111111111111)
    for trial in range(args.ntrials):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            # print("[TRAIN] Epoch:{:04d} | klLoss {:.4f} | Class loss:{:.4f} | TRAIN ACC:{:.4f} | f1_train:{:.2f}| auc_train:{:.2f}| precision_train:{:.2f}| | Training duration: {:.6f}".\
            #       format(epoch, kl_loss, class_loss, acc_train, f1_train, auc_train, precision_train, train_epoch_time_duration))
    ...
    np.save('./model_save/DGA-200/best_idx11.txt', test_idx)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-lr_clas', type=float, default=0.001)#学习率0.001
